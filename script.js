...
const MAX_TURNS = 5; // feels like a real short conversation
...
async function loadFirstQuestion(){
  ...
  const first = list[Math.floor(Math.random()*Math.max(1,list.length))] || "¿Puedes hablar un poco de este tema?";
...
async function onSubmit(){
  ...
  // display per-turn feedback (short, examiner style)
...
function renderSummary(result){
  ...
  updateBest(state.theme, localStorage.getItem("oral_tense") || "present", sessionStars);
  ...
  out.innerHTML = `
    ...
    <div style="margin-top:10px;">
      ...
      <ul style="margin:8px 0 0 18px;">
        ${(drills.length ? drills : ["Add 1 reason (porque) + 1 extra detail in every answer.", "Use one connector: además / también / sin embargo.", "Repeat the same theme again and aim for +10 points."]).map(d=>`<li>${escapeHtml(String(d))}</li>`).join("")}
      </ul>
    </div>
    <div style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap;">
